...
export default function ActionPlan() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Progress Banner */}
      <div className="bg-gradient-to-r from-navy-800 to-navy-700 text-white rounded-xl p-5">
        ...
        <div className="flex flex-wrap gap-3 text-xs text-navy-300">
      ...
      <div className="space-y-2">
        ...
        {filtered.map(item => (
          <div key={item.id} className={`bg-white border rounded-xl px-4 py-3 transition-all hover:shadow-sm ${item.status === 'done' ? 'opacity-60' : ''}`}>
            <div className="flex items-start gap-3">
              ...
              <button onClick={() => deleteItem(item.id)} className="text-gray-300 hover:text-red-500 text-sm shrink-0 mt-0.5">✕</button>
            </div>
          </div>
        ))}
      </div>
